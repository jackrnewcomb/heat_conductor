...
bool sanitizeArgMap(const std::map<std::string, std::string>& map) {
  ...
  for (const auto& pair : map) {
    ...
    // If it fails, print an error message and abort
      ...
      std::cout << "Invalid input for " << pair.first
                << ": Value must cast to an int. Aborting!\n";
      return false;
    }
  }
  return true;
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    // If the arg doesn't contain "-", the user did not format their arguments
    // correctly. Pass a message to the console and move on
    else {
