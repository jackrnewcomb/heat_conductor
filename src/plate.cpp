...
Plate::Plate(int points) {
  ...
  auto number_of_filament_points =
      int(4 * (10.0 / pointsPerSide));  // number of filament points isn't an
                                        // int...rounding for now

  auto mid_point = int(pointsPerSide / 2);
  std::vector<int> filament_index_spread{
      mid_point - (number_of_filament_points / 2),
      mid_point + (number_of_filament_points / 2)};

  for (int i = 0; i < pointsPerSide; i++) {
    for (int j = 0; j < pointsPerSide; j++) {
      if ((i >= filament_index_spread[0] && i <= filament_index_spread[1]) &&
          j == pointsPerSide - 1) {
        grid_[i * pointsPerSide + j] = 100;
      } else if (i == 0 || i == pointsPerSide - 1 || j == 0 ||
                 j == pointsPerSide - 1) {
...
void Plate::updatePoint(int i, int j) {
  ...
  // std::cout << "Based on temps " << grid_[i - 1][j] <<", " << grid_[i + 1][j]
  // << ", " << grid_[i ][j-1] << ", and " << grid_[i][j+1] << "...new temp = "
  // << newTemp << "\n";
...
void Plate::update() {
  ...
  for (int i = 1; i < pointsPerSide - 1; i++) {
    for (int j = 1; j < pointsPerSide - 1; j++) updatePoint(i, j);
  }
