...
/**
 * @brief Plate constructor. Takes in an int representing the total number of
 * points in the grid, and constructs a Plate
...
Plate::Plate(int points) {
  ...
  auto numberOfFilamentPoints =
      int((filamentLength / sideLength) * pointsPerSide);
  ...
  // Work through the grid, and if a point is an edge point, append it with the
  // proper starting temperature
  for (int i = 0; i < pointsPerSide; i++) {
    for (int j = 0; j < pointsPerSide; j++) {
      // If its a filament edge point...
      if ((i >= filamentIndexSpread[0] && i <= filamentIndexSpread[1]) &&
          j == pointsPerSide - 1) {
